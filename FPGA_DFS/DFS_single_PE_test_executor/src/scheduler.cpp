...
#pragma HLS INTERFACE axis port=axis_join_finish
    ...
    for (int i = 0; i < iter_num; i++) {
        ...
        if (data_join) {
            ...
            pair_t result = block_read<pair_t>(axis_page_pair_scheduler);
            count += result.id_A;
            axis_page_ID_pair_read_nodes.write(p);
        }
        // for joins of non-data pairs -> send to layer cache write request
        else {
            axis_read_write_control.write(1);    
            axis_write_layer_id.write(0);
            int result = block_read<int>(axis_intersect_count_directory_scheduler);
            count += result;
        }
    }
    axis_join_finish.write(count);
