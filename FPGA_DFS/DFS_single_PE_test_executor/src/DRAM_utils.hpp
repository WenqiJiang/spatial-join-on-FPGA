...
void read_nodes(
    ...
    ////// debug starts
    pair_t reg_p = block_read<pair_t>(axis_page_ID_pair_read_nodes);
    node_meta_t reg_n;
    obj_t reg_o; 
    reg_n.is_leaf = reg_p.id_A + in_pages_A[0].range(31,0) + in_pages_B[0].range(31,0);
    reg_o.id = reg_p.id_A;
    ...
    while (s_join_finish_replicated.empty()) {}
...
void layer_cache_memory_controller(
    ...
    int read = block_read<int>(axis_read_write_control);
    int layer_id = block_read<int>(axis_read_layer_id);
    int layer_pointer = block_read<int>(axis_read_layer_pointer);
    int sum = read + layer_id + layer_pointer + layer_cache[0].range(31,0);
    pair_t pair;
    pair.id_A = sum;
    axis_page_pair_scheduler.write(pair);

    // get write requests, and write results
    int write = block_read<int>(axis_read_write_control) + block_read<int>(axis_write_layer_id);
    while (s_intersect_count_directory.empty()) {}
    int count = s_intersect_count_directory.read() + write;
    axis_intersect_count_directory_scheduler.write(count);
    while (s_result_pair_directory.empty()) {}
    ...
    while (true) {
        ...
        if (!s_join_finish_replicated.empty()) {
            ...
            if (write) {
                int write_layer_id = block_read<int>(axis_write_layer_id);
                ...
                int read_layer_id = block_read<int>(axis_read_layer_id);
                int read_layer_pointer = block_read<int>(axis_read_layer_pointer);
...
void write_results(
    ...
    ////// debug starts
    while (s_intersect_count_leaf.empty()) {}
    int reg0 = s_intersect_count_leaf.read();
    while (s_result_pair_leaf.empty()) {}
