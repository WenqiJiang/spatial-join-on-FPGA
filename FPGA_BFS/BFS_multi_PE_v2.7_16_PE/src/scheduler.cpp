...
void scheduler(
    ...
    int max_level_A,
    int max_level_B,
	int max_active_task_per_PE, // the maximum active tasks per PE
    //   from join PE
...
#pragma HLS INTERFACE axis port=axis_join_finish
    ...
    int join_PE_active_tasks[N_JOIN_PE] = {0}; // 0 -> idle; i -> i tasks in the pipeline
    int join_PE_task_count[N_JOIN_PE] = {0}; // for tracking the number of tasks allocated to each PE

    const int pair_cache_size = 128;
    ...
    for (int current_level = 1; current_level < max_level + 1; current_level++) {
		...
		int join_PE_to_check = 0; // check whether this PE is idle, do this in a round robin manner
        ...
        for (int join_id = 0; join_id < num_pairs_per_level[current_level - 1]; join_id++) {
            ...
            while (true) {
                ...
                while (!axis_idle_join_PE_ID.empty()) { // set as idle
                    ...
                    join_PE_active_tasks[idle_join_PE_ID]--; 
                }

				// find a round for an idle PE
                for (int round_cnt = 0; round_cnt < N_JOIN_PE; round_cnt++) {

					if (join_PE_to_check == N_JOIN_PE) {
						join_PE_to_check = 0;
					}
					if (join_PE_active_tasks[join_PE_to_check] < max_active_task_per_PE) {
					
                        // send the node pair and join PE id to the read PE
                        axis_page_ID_pair_read_nodes.write(node_pairs);   
                        axis_join_PE_ID.write(join_PE_to_check);
                        join_PE_active_tasks[join_PE_to_check]++;
                        join_PE_task_count[join_PE_to_check]++;
						join_PE_to_check++;
                        break_while = true; 
                        break;	
					} else {
