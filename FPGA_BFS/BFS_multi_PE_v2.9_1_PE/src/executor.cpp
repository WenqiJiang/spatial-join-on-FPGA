...
#pragma HLS dataflow
	...
	hls::stream<pair_t> s_page_ID_pair_read_nodes[2];
#pragma HLS stream variable=s_page_ID_pair_read_nodes depth=512

	hls::stream<int> s_join_PE_ID;
#pragma HLS stream variable=s_join_PE_ID depth=512

    hls::stream<ap_uint<512> > s_page_A_raw_unredirected;
#pragma HLS stream variable=s_page_A_raw_unredirected depth=512
    hls::stream<ap_uint<512> > s_page_B_raw_unredirected;
#pragma HLS stream variable=s_page_B_raw_unredirected depth=512
...
#pragma HLS stream variable=s_finish_scheduler_out depth=2
    ...
    hls::stream<int> s_finish_forward_AXIS_read_signals_out; 
#pragma HLS stream variable=s_finish_forward_AXIS_read_signals_out depth=2

    hls::stream<int> s_finish_forward_AXIS_read_signals_out_replicated[2]; 
#pragma HLS stream variable=s_finish_forward_AXIS_read_signals_out_replicated depth=2

    hls::stream<int> s_finish_read_nodes_out[2]; 
#pragma HLS stream variable=s_finish_read_nodes_out depth=2

    hls::stream<int> s_finish_read_nodes_out_aggregated; 
#pragma HLS stream variable=s_finish_read_nodes_out_aggregated depth=2

    hls::stream<int> s_finish_forward_input_to_join_PEs_out; 
#pragma HLS stream variable=s_finish_forward_input_to_join_PEs_out depth=2

    hls::stream<int> s_finish_forward_input_to_join_PEs_out_replicated[N_JOIN_PE]; 
#pragma HLS stream variable=s_finish_forward_input_to_join_PEs_out_replicated depth=2
...
#pragma HLS stream variable=s_finish_layer_cache_out depth=2
	...
	forward_AXIS_read_signals(
		...
		s_join_PE_ID,
		s_finish_forward_AXIS_read_signals_out);

    broadcast_finish_signals<2>(
        s_finish_forward_AXIS_read_signals_out,
        s_finish_forward_AXIS_read_signals_out_replicated);
	...
	read_nodes_A(
		...
		in_pages_A,
		s_page_ID_pair_read_nodes[0],
		s_finish_forward_AXIS_read_signals_out_replicated[0],
		// output
		s_page_A_raw_unredirected,
		s_finish_read_nodes_out[0]); 
	...
	read_nodes_B(
		...
		max_entry_num, // set during indexing
		in_pages_B,
		s_page_ID_pair_read_nodes[1],
		s_finish_forward_AXIS_read_signals_out_replicated[1],
		// output
		s_page_B_raw_unredirected,
		s_finish_read_nodes_out[1]); 

    aggregate_finish_signals<2>(
        s_finish_read_nodes_out,
        s_finish_read_nodes_out_aggregated); 

	forward_input_to_join_PEs(
		// input
		page_bytes, // typically fixed as 4096
		max_entry_num, // set during indexing
		s_join_PE_ID,
		...
		s_page_A_raw,
		s_page_B_raw,
		s_finish_forward_input_to_join_PEs_out);


    broadcast_finish_signals<N_JOIN_PE>(
        s_finish_forward_input_to_join_PEs_out,
        s_finish_forward_input_to_join_PEs_out_replicated);
...
#pragma HLS unroll
		parse_page(
			...
			s_page_A_raw[PE_id],
			s_page_B_raw[PE_id],
			s_finish_forward_input_to_join_PEs_out_replicated[PE_id],
			// output
			s_meta_A[PE_id],
			s_meta_B[PE_id],
			s_page_A[PE_id],
