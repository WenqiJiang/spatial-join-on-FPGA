...
void forward_AXIS_read_signals(
	// in
    ...
    while (true) {
        ...
        if (!axis_page_ID_pair_read_nodes.empty()) {
			...
			s_page_ID_pair_read_nodes[0].write(page_ID_pair);
			s_page_ID_pair_read_nodes[1].write(page_ID_pair);
			s_join_PE_ID.write(join_PE_ID);
...
// Read only one of the two pages
void read_nodes_A(
    // input
    ...
    while (true) {
        ...
        if (!s_page_ID_pair_read_nodes.empty()) {
...
#pragma HLS pipeline II=1
                s_page_A_raw_unredirected.write(in_pages_A[start_addr_A + i]);
            }
        } else if (!s_join_finish_in.empty()) {
            int end = s_join_finish_in.read();
            s_join_finish_out.write(end);
...
void read_nodes_B(
    ...
    while (true) {
        ...
        if (!s_page_ID_pair_read_nodes.empty()) {
            ...
            for (int i = 0; i < axi_per_page; i++) {
#pragma HLS pipeline II=1
                s_page_B_raw_unredirected.write(in_pages_B[start_addr_B + i]);
            }
        } else if (!s_join_finish_in.empty()) {
            int end = s_join_finish_in.read();
            s_join_finish_out.write(end);
...
// given the raw input streams, forward them to the respective PE
void forward_input_to_join_PEs(
    // input
    const int page_bytes, // typically fixed as 4096
	const int max_entry_num, // set during indexing
    hls::stream<int>& s_join_PE_ID,
    ...
    while (true) {
        ...
        if (!s_join_PE_ID.empty()) {

            int join_PE_ID = s_join_PE_ID.read();
			// always read the all content up to the max entry size
            for (int i = 0; i < axi_per_page; i++) {
#pragma HLS pipeline II=1
                s_page_A_raw[join_PE_ID].write(s_page_A_raw_unredirected.read());
                s_page_B_raw[join_PE_ID].write(s_page_B_raw_unredirected.read());
                }
        } else if (!s_join_finish_in.empty()) {
            int end = s_join_finish_in.read();
            s_join_finish_out.write(end);
