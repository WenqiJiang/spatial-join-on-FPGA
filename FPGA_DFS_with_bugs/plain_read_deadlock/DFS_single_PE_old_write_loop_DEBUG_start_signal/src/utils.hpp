...
void start_handler(
    // in
    hls::stream<int>& s_scheduler_start, // from scheduler
    // out
    hls::stream<int>& s_executer_start,  // send out
    hls::stream<int> (&s_start)[num_stream] // for internal PEs
) {

    s_executer_start.write(1);

    while (s_scheduler_start.empty()) {}
    // wait for 100 cycles for other signals to cone
    volatile int idle_count = 0;
    for (int i = 0; i < 100; i++) {idle_count++;}
    int start = s_scheduler_start.read();    

    for (int s = 0; s < num_stream; s++) {
#pragma HLS unroll
        s_start[s].write(start);
    }
}
