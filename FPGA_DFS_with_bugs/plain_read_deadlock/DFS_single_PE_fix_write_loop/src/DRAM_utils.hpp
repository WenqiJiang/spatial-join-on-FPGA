...
void read_nodes(
    ...
    ////// debug starts
    while (s_page_ID_pair_read_nodes.empty()) {}
...
void layer_cache_memory_controller(
    ...
    while (s_read_write_control.empty()) {}
    int read = s_read_write_control.read();
    while (s_read_layer_id.empty()) {}
    int layer_id = s_read_layer_id.read();
    while (s_read_layer_pointer.empty()) {}
