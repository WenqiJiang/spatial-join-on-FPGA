...
void read_nodes(
    ...
    // write FIFO
    node_meta_t reg_node_meta_t;
    obj_t reg_obj_t;
    s_meta_A.write(reg_node_meta_t);
    s_meta_B.write(reg_node_meta_t);
    s_page_A.write(reg_obj_t);
    s_page_B.write(reg_obj_t);

    // read AXI
    volatile pair_t reg_p = block_read<pair_t>(axis_page_ID_pair_read_nodes);

    // read FIFO
    volatile int reg_int = s_join_finish_replicated.read();
}
...
void layer_cache_memory_controller(
    ...
    // read FIFO
    volatile int reg_A = s_intersect_count_directory.read();
    volatile result_t reg_B = s_result_pair_directory.read(); 
    volatile int reg_C = s_join_finish_replicated.read();
}
...
void write_results(
    ...
    // read FIFO 
    volatile int reg_A = s_intersect_count_leaf.read();
    volatile result_t reg_B = s_result_pair_leaf.read();
    volatile int reg_C = s_join_finish_replicated.read();
