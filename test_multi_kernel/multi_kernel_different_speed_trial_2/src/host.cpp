...
// OPENCL HOST CODE AREA START
    ...
    OCL_CHECK(err, err = krnl_A.setArg(3, buffer_output));

    // int arg_counter_B = 0;    
    OCL_CHECK(err, err = krnl_B.setArg(0, total_iter));
    OCL_CHECK(err, err = krnl_B.setArg(1, sleep_delay));
    OCL_CHECK(err, err = krnl_B.setArg(2, buffer_input));
