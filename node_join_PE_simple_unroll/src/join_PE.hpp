...
static const int join_unroll_factor = JOIN_UNROLL_FACTOR;
...
void join_page(
    ...
    hls::stream<result_t> (&s_result_pair_array)[JOIN_UNROLL_FACTOR]) {

    obj_t page_A[MAX_PAGE_ENTRY_NUM];
    obj_t page_B[MAX_PAGE_ENTRY_NUM];
#pragma HLS array_partition variable=page_A factor=join_unroll_factor
#pragma HLS array_partition variable=page_B factor=join_unroll_factor
...
#pragma HLS pipeline II=1
#pragma HLS unroll factor=join_unroll_factor
                ...
                if (overlap) {
                    ...
                    s_result_pair_array[n % join_unroll_factor].write(result);
                }
            }
        }
