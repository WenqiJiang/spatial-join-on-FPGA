...
// Wenqi: this function can become the performance bottleneck, as we need 
//   to check each FIFO in round-robin manner
void unify_results(
    hls::stream<result_t> (&s_result_pair_array)[JOIN_UNROLL_FACTOR],
    hls::stream<result_t>& s_result_pair) {

    while (true) {
        for (int s = 0; s < JOIN_UNROLL_FACTOR; s++) {
            if (!s_result_pair_array[s].empty()) {
                s_result_pair.write(s_result_pair_array[s].read());
            }
        }
    }
}
